...
const emptyProject = (): Project => ({
    ...
    liveUrl: '', githubUrl: '', imageUrl: '', category: '',
});

const inputFields = [
    { name: 'title', label: 'Project Title', placeholder: 'e.g. Portfolio Website' },
    { name: 'category', label: 'Category', placeholder: 'e.g. Full Stack, Frontend, AI...' },
    { name: 'technologies', label: 'Technologies', placeholder: 'React, Node.js, MongoDB...' },
    { name: 'liveUrl', label: 'Live URL', placeholder: 'https://yourproject.com' },
    { name: 'githubUrl', label: 'GitHub URL', placeholder: 'https://github.com/user/repo' },
    { name: 'imageUrl', label: 'Image URL', placeholder: 'https://i.imgur.com/example.png' },
];

const EditProjects: React.FC = () => {
    const [items, setItems] = useState<Project[]>([emptyProject()]);
    ...
    useEffect(() => {
        axios.get(`${API_BASE_URL}/api/projects`)
            .then((r) => setItems(r.data.length > 0 ? r.data : [emptyProject()]))
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await axios.put(`${API_BASE_URL}/api/admin/projects`, { projects: items }, {
        ...
        <AdminLayout>
            <div className="max-w-2xl">
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <button
                        type="button"
                        onClick={() => setItems((p) => [...p, emptyProject()])}
                        ...
                        {items.map((item, index) => (
                            <div key={index} className="bg-gray-900 border border-white/10 rounded-xl p-6">
                                <div className="flex items-center justify-between mb-4">
                                    <span className="text-sm font-semibold text-slate-300">Project #{index + 1}</span>
                                    {items.length > 1 && (
                                        <button
                                            type="button"
                          ...
                          text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all"
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Describe the project, key features, your role..."
                                            className="w-full px-3 py-2.5 rounded-lg bg-gray-800 border border-white/10 text-white placeholder-slate-500
